...
这是一个基于 Python 的图像标注数据（Labelme 格式）处理工具箱，提供图形化界面整合五个工具：

| 工具 | 文件 | 功能 | 状态 |
|------|------|------|------|
| 1. 文件计数与匹配统计 | `tools/image_count.py` | 统计图片/JSON配对数量 | ✅ 已完成 |
| 2. 孤立文件清理 | `tools/orphan_image_cleaner.py` | 清理孤立的图片/JSON文件 | ✅ 已完成 |
| 3. 标签正确性检查 | `tools/label_validator.py` | 校验JSON中的标签是否合法 | ✅ 已完成 |
| 4. 多边形重叠检查 | `tools/polygon_overlap_checker.py` | 检测多边形标注重叠并导出问题副本与报告 | ✅ 已完成 |
| 5. 检查抽样 | `tools/image_json_sampler.py` | 从数据集中抽样检查 | ✅ 已完成 |
...
# 工具模块导入测试
python -c "from tools.image_json_sampler import run_sampler; from tools.label_validator import run_validator; from tools.polygon_overlap_checker import run_polygon_overlap_check; print('OK')"
```
...
| `get_leaf_folders(root_path)` | 获取所有叶子文件夹 | `list[str]` |
| `scan_leaf_dir(leaf_dir)` | 扫描单个叶子文件夹配对情况 | `dict` |
| `scan_all_leaf_dirs(root_path)` | 扫描所有叶子文件夹汇总统计 | `dict` |
| `find_orphans_in_leaf(leaf_dir)` | 查找叶子文件夹中的孤立文件 | `dict` |
| `find_all_orphans(root_path)` | 扫描所有叶子文件夹的孤立文件 | `dict` |
...
7. **扫描排除策略**：新增递归扫描逻辑时，优先复用 `core/file_scanner.py` 中的公共扫描函数，并排除结果目录（如 `ERROR_CHECK_RESULTS`、抽样输出目录）
